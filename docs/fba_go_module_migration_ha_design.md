...
│   └── task/                   # 可选：内置 task 插件
├── cmd/
│   └── fbagen/                 # 插件扫描、DI 生成、Swagger 聚合、契约检查
├── contracts/
...
```makefile
generate:
    fbagen plugin scan --mode imports,local,manifest --plugins-dir ./plugins --manifest ./plugins.yaml --out internal/generated/fba_plugins.gen.go
    fbagen plugin graph --out internal/generated/plugin_graph.dot
    fbagen swagger scan --out docs/openapi.json
    fbagen contract snapshot --out internal/generated/api.contract.snapshot.json
```
...
| Dynamic import | fbagen build-time code generation |
...
fbagen contract test
fbagen migration plan
fbagen migration apply
```
